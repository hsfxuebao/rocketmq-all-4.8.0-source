...
public class CommitLog {
    ...
    public void recoverNormally(long maxPhyOffsetOfConsumeQueue) {
        ...
        // 从倒数第3个文件开始恢复，如果不足3个文件，则从第一个文件开始恢复
        if (!mappedFiles.isEmpty()) {
            ...
            while (true) {
                ...
                // Normal data
                // 如果查找结果为true并且消息的长度大于0，表示消息正确，mappedFileOffset指针
                //向前移动本条消息的长度
                ...
                // 如果查找结果为false，表明该文件未填满所有消息，则跳出循环，结束遍历文件
            ...
            // 更新MappedFileQueue的flushedWhere和committedPosition指针
    ...
    public void recoverAbnormally(long maxPhyOffsetOfConsumeQueue) {
        ...
        if (!mappedFiles.isEmpty()) {
            ...
            long processOffset = mappedFile.getFileFromOffset();
            long mappedFileOffset = 0;
            // 则遍历MappedFile中的消息，验证消息的合法性，并将消息重新转发到ConsumeQueue与
            //Index文件
            while (true) {
                // 取出消息
                DispatchRequest dispatchRequest = this.checkMessageAndReturnSize(byteBuffer, checkCRCOnRecover);
        ...
        // Commitlog case files are deleted
        // 如果未找到有效的MappedFile，则设置CommitLog目录的
    ...
    private boolean isMappedFileMatchedRecover(final MappedFile mappedFile) {
        ...
        if (this.defaultMessageStore.getMessageStoreConfig().isMessageIndexEnable()
            ...
            // todo 如果文件第一条消息的时间戳小于文件检测点，说明该文件的部分消息是可靠的，
            // 则从该文件开始恢复
    ...
    public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
        ...
        try {
            ...
            // 若mappedFile 还未null 说明磁盘空间不足或权限不够导致的，直接抛异常
            if (null == mappedFile) {
                log.error("create mapped file1 error, topic: " + msg.getTopic() + " clientAddr: " + msg.getBornHostString());
                beginTimeInLock = 0;
    ...
    public SelectMappedBufferResult getMessage(final long offset, final int size) {
        int mappedFileSize = this.defaultMessageStore.getMessageStoreConfig().getMappedFileSizeCommitLog();
        // todo 根据消息偏移量offset查找MappedFile
    ...
    class CommitRealTimeService extends FlushCommitLogService {
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                ...
                try {
                    // todo 执行提交操作，将待提交数据提交到物理文件的内存映射内存区
    ...
    class GroupCommitService extends FlushCommitLogService {
        ...
        /**
         * 为了避免同步刷盘消费任务与其他消息生产者提交任务产生锁竞
         * 争，GroupCommitService提供读容器与写容器，这两个容器每执行完
         * 一次任务后交互，继续消费任务
         */
